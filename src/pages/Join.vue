    ...
    <q-card flat :bordered="$q.screen.gt.xs" class="join" :class="$q.screen.gt.xs ? 'q-pa-lg' : 'q-pa-none'">
      ...
      <q-card-section>
        <q-form autocomplete="on" class="column q-gutter-y-md" @submit="join">
          <q-input outlined no-error-icon hide-bottom-space :readonly="email !== null" v-model="info.email" type="email"
            maxlength="320" :rules="[val => val && val.length > - 6 && checkEmail(val) || '']" label="이메일" />
          <q-input outlined no-error-icon hide-bottom-space v-model="info.pw" type="password" maxlength="16"
            :rules="[val => val && val.length >= 8 && complexity.value >= 60 || '']" label="비밀번호"
          ...
          <q-input outlined no-error-icon hide-bottom-space v-model="cpw" type="password" maxlength="16"
            :rules="[val => val && info.pw === val || '']" label="비밀번호 확인" />
          <q-btn outline :ripple="false" text-color="secondary" class="bg-primary shadow-1 text-weight-bold" label="가입"
            padding="md" type="submit" />
        </q-form>
      </q-card-section>
    </q-card>
  </div>
