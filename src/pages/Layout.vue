...
const asideHeight = computed(
  () => `calc(100vh - ${screen.value.gt.sm ? offsetTop.value : 0}px)`
)
...
const setLang = (lang) => {
  router
    .replace({ name: route.name, params: { lang } })
    .catch(() => {})
    .then(() => {
      router.go(0)
    })
...
const reload = () => {
  if (
    Date.now() - globalStore.topAccessTimeStamp > globalStore.timeLimit ||
    topRef.value?.$el?.getAttribute('data-ad-status') === 'unfilled'
  ) {
    topKey.value++
  ...
  if (
    Date.now() - globalStore.bottomAccessTimeStamp > globalStore.timeLimit ||
    bottomRef.value?.$el?.getAttribute('data-ad-status') === 'unfilled'
  ) {
    bottomKey.value++
  ...
  if (
    Date.now() - globalStore.rightAccessTimeStamp > globalStore.timeLimit ||
    rightRef.value?.$el?.getAttribute('data-ad-status') === 'unfilled'
  ) {
    rightKey.value++
...
<template>
  <q-layout view="hHh lpR lFf">
    ...
    <q-header
      bordered
      class="bg-white text-secondary q-py-md header row justify-center"
    >
      <q-toolbar class="toolbar">
        <div class="col-3 row items-center">
          <q-btn
            class="gt-sm no-hover"
            dense
            flat
            padding="0"
            :ripple="false"
        ...
        <q-btn-group
          class="col-3 justify-end"
          :class="screen.gt.sm ? 'q-gutter-x-md' : 'q-gutter-x-sm'"
          rounded
          flat
        >
          <q-btn flat dense :ripple="false">
            ...
            >
              <q-list bordered class="rounded-borders">
                <q-item
                  v-for="lang in globalStore.localeOptions"
                  :key="lang.value"
                  clickable
          ...
          <q-btn
            ...
            dense
            :ripple="false"
            :to="{ name: 'Info', params: { lang: route.params.lang } }"
          >
      ...
      <q-page :style-fn="myTweak">
        <div class="row justify-center">
          <div
            ...
            :style="screen.lt.sm ? 'width:100%' : 'width:824px'"
          >
            <div class="row justify-center top-ads">
              <Adsense
                v-if="!accountStore.noAds"
                ref="topRef"
                :style="`display:inline-block;${size}`"
                data-ad-slot="3025920602"
                :data-adtest="!prod"
                :key="topKey"
              />
            </div>
